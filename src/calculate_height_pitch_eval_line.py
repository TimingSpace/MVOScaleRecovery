...
from estimate_road_norm import estimate_line
...
#flag
    ...
    while frame_count<frame_number:
        ...
        if(point_selected.shape[0]>=12):
            ...
            m_2,b_2 = get_pitch_line_ransac(a_array[:,1:],iter_number,0.005)
            #print 'road model ransac',m_2
            #print 'inlier number',b_2
            inlier_id = get_inliers(m_2,a_array[:,1:],0.01)
            #print inlier_id
            inliers = a_array[inlier_id,:]
            inlier_id_all = get_inliers(m_2,points_3d_array[:,1:],0.01)
            inliers_2d = points[inlier_id_all,:]
            outliers_2d = points[inlier_id_all==False,:]

            road_model_ransac = np.matrix(m_2)
        ...
        start_time = time.time()
        # refine n
        refined_norm = estimate_line(inliers[:,1:])
        ...
        refined_pitch = math.asin(refined_norm[0,0])
        inliers_matrix = np.matrix(inliers)
        refined_pitchs.append(refined_pitch)
        # refine h
        refined_hs = inliers_matrix[:,1:]*refined_norm.T
...
#save data
    ...
    np.savetxt('eval_ransac_line/result_heights_plane_ransac_'+input_id+'_'+input_date+'_'+str(iter_number)+'_'+str(case_i)+'.txt',ransac_camera_heights)

    refined_camera_height_means = np.array(refined_camera_height_means)
    np.savetxt('eval_ransac_line/refined_camera_height_means'+input_id+'_'+input_date+'_'+str(iter_number)+'_'+str(case_i)+'.txt''.txt',refined_camera_height_means)

    refined_camera_height_stds =np.array(refined_camera_height_stds)
    np.savetxt('eval_ransac_line/refined_camera_height_stds'+input_id+'_'+input_date+'_'+str(iter_number)+'_'+str(case_i)+'.txt''.txt',refined_camera_height_stds)

    refined_camera_height_t_means = np.array(refined_camera_height_t_means)
    np.savetxt('eval_ransac_line/refined_camera_height_t_means'+input_id+'_'+input_date+'_'+str(iter_number)+'_'+str(case_i)+'.txt''.txt',refined_camera_height_t_means)

    refined_pitchs = np.array(refined_pitchs)
    np.savetxt('eval_ransac_line/refined_pitch'+input_id+'_'+input_date+'_'+str(iter_number)+'_'+str(case_i)+'.txt''.txt',refined_pitchs)
