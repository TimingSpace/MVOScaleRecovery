...
camera_motion_path = sys.argv[3]
camera_pose_path = sys.argv[4]
...
while frame_count<4541:
    ...
    if(point_selected.shape[0]>=12):
        ...
        a_array[:,0] = a_array[:,2]*(a_array[:,0]-camera_cx)/camera_focus
        a_array[:,1] = a_array[:,2]*(a_array[:,1]-camera_cy)/camera_focus
        m,b = get_pitch_ransac(a_array,500,0.005)
        m_2,b_2 = get_pitch_line_ransac(a_array[:,1:],500,0.005)
        print 'road model ransac',m_2
        print 'inlier number',b_2
        inlier_id = get_inliers(m,points_3d_array[:,:],0.01)
        inliers = points_3d_array[inlier_id,:]
        inliers_2d = points[inlier_id,:]
        outliers_2d = points[inlier_id==False,:]
...
np.savetxt('result_heights_line_ransac.txt',ransac_camera_heights)

refined_camera_height_means = np.array(refined_camera_height_means)
np.savetxt('refined_camera_height_means.txt',refined_camera_height_means)

refined_camera_height_stds =np.array(refined_camera_height_stds)
np.savetxt('refined_camera_height_stds.txt',refined_camera_height_stds)

refined_camera_height_t_means = np.array(refined_camera_height_t_means)
np.savetxt('refined_camera_height_t_means.txt',refined_camera_height_t_means)

refined_pitchs = np.array(refined_pitchs)
np.savetxt('refined_pitch.txt',refined_pitchs)

inlier_numbers = np.array(inlier_numbers)
np.savetxt('inlier_numbers.txt',inlier_numbers)
